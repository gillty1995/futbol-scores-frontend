...
function GamesSection() {
  ...
  useEffect(() => {
    ...
    try {
      // Find the team from league standings
      const teamData = leagueStandings.find(
      ...
      // Filter games based on the teamId from matchResults
      const filteredGames = matchResults.filter(
        (game) =>
          game.homeTeam.id === parseInt(teamId) ||
          game.awayTeam.id === parseInt(teamId)
      );
